...
Astra helps you manage stateful Kubernetes workloads with a set of storage and application-aware data management services powered by NetApp data protection technology. Easily protect, back up, and migrate Kubernetes workloads, and instantly create working application clones.

Astra offerings include:

* *Astra Control Service*: Use a service managed by NetApp for app-aware data management of Kubernetes workloads in public clouds.
* *Astra Control Center*: Use self-managed software for application-aware data management of on-premises Kubernetes workloads.
* *Astra Data Store*: Use Kubernetes native shared file service for both container and VM workloads delivering enterprise data management.
* *Astra Trident*: Use CSI compliant storage provisioning and management for Kubernetes workloads with NetApp storage providers.
...
NetApp has validated some apps to ensure the safety and consistency of the snapshots and backups. https://docs.netapp.com/us-en/astra-control-center/concepts/validated-vs-standard.html[Learn the difference between a validated app and a standard app].
...
** Uses that new admin role to install https://docs.netapp.com/us-en/trident/index.html[NetApp's Trident^] on the cluster and to create one or more storage classes.
+
